...
const QVector<QString> HEADERS{QObject::tr("Templates")};
...
bool TemplateManager::handleExtension(Extension* extension) {
    ...
    if (tmpProvider != nullptr) {
        ...
        foreach (auto& tmpl, templates) { addTemplate(tmpl); }
    }
    return true;
}
...
bool TemplateManager::loadCoreTemplates() {
    ...
    foreach (auto& tmpl, tmps) { addTemplate(tmpl); }
    endResetModel();
    return true;
}
