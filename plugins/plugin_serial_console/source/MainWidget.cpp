...
struct MainWidget::Data {
    Data(QWidget* parent)
        ...
        , m_newConnection{new QAction(getIcon(FAIcon::Plug),
                                      QObject::tr("New Connection"),
                                      parent)}
        , m_editBaudRates{new QAction(getIcon(MatIcon::Edit),
                                      QObject::tr("Edit Baud Rates"),
                                      parent)}
        , m_disconnectAll{new QAction(getIcon(MatIcon::Clear),
...
MainWidget::MainWidget(QWidget* parent)
    ...
    connect(m_data->m_tabWidget,
            ...
            this,
            &MainWidget::createNewConnection);
    connect(m_data->m_editBaudRates, &QAction::triggered, [&]() {
        Dialogs::openBaudRateEditor();
    });
    connect(
        m_data->m_tabWidget, &QTabWidget::tabCloseRequested, [this](int index) {
