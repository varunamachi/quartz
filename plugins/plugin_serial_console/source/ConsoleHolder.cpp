...
using SerialErrorFunc = void (QSerialPort::*)(QSerialPort::SerialPortError);

using ComboIdxFunc = void (QComboBox::*)(int);

struct ConsoleHolder::Data {
    explicit Data(std::unique_ptr<SerialSettings> settings, QWidget* parent);
...
ConsoleHolder::Data::Data(std::unique_ptr<SerialSettings> settings,
    ...
    , m_connect{new QAction(getIcon(FAIcon::Plug),
                            QObject::tr("Connect"),
                            parent)}
    , m_disconnect{new QAction(getIcon(MatIcon::Block),
                               QObject::tr("Disconnect"),
                               parent)}
    , m_clearConsole{new QAction(getIcon(FAIcon::Broom),
                                 QObject::tr("Clear Input"),
                                 parent)}
    , m_lockOutput{new QAction(getIcon(MatIcon::Lock),
                               QObject::tr("Lock"),
                               parent)}
...
void ConsoleHolder::Data::updateCompleteInfo() {
    ...
    infoStream << "<b>Information:</b><br>" << I << tr("Port Name") << ": <b>"
               << info.portName() << "</b><br>" << I << tr("Description")
               << ": <b>" << info.description() << "</b><br>" << I
               << tr("Serial Number") << ": <b>" << info.serialNumber()
               << "</b><br>" << I << tr("Location") << ": <b>"
               << info.systemLocation() << "</b><br>" << I << tr("VID")
               << ": <b>" << int(info.vendorIdentifier()) << "</b><br>" << I
               << tr("PID") << ": <b>" << int(info.productIdentifier())
               << "</b><br>"
               << "<br><b>Settings:</b><br>" << I << tr("Baud Rate") << ": <b>"
               << m_settings->baudRate() << "</b><br>" << I << tr("Data Bits")
               << ": <b>" << m_settings->dataBits() << "</b><br>" << I
               << tr("Parity") << ": <b>"
               << SerialUtils::decodeParity(m_settings->parity()) << "</b><br>"
               << I << tr("Stop Bits") << ": <b>"
               << SerialUtils::decodeStopBits(m_settings->stopBits())
               << "</b><br>" << I << tr("Flow Control") << ": <b>"
...
ConsoleHolder::ConsoleHolder(std::unique_ptr<SerialSettings> settings,
    ...
    connect(m_data->m_lockOutput, &QAction::toggled, [this](bool val) {
    ...
    connect(m_data->m_serial, &QSerialPort::readyRead, [this]() {
    ...
    connect(m_data->m_console,
            ...
            [this](const QByteArray& data) {
                auto html = "<br>Command: <font color=red><b>" + QString(data)
                    + "</b></font><br>"
                    + "---------------------------------------------------<br>";
...
bool ConsoleHolder::connectSerial() {
    ...
    if (!m_data->m_serial->open(QIODevice::ReadWrite)) {
        ...
        QMessageBox::critical(
            this, tr(" Serial Port Error"), m_data->m_serial->errorString());
    } else {
