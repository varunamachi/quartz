...
void parseOptions(ChoiceParam* param,
    ...
    for (auto opt = choiceNode.firstChildElement("option"); !opt.isNull();
        ...
        if (!name.isEmpty() && !val.isEmpty()) {
            ...
            QZ_ERROR("Qz:Cmn:GenParam")
                << "Invalid option found for choice param " << param->name();
        }
    }
}
...
QVector<std::shared_ptr<Config>> ConfigParser::parse(
    ...
    if (content.isNull()) {
        QZ_ERROR("Qz:Cmn:GenParam") << "Invalid content given to general param "
                                       "parser";
        return configs;
    }
    QDomDocument doc;
        ...
        if (root.tagName() == "configs") {
            ...
            for (auto i = 0; i < configLists.size(); ++i) {
                ...
                if (config != nullptr) {
                    ...
                    QZ_ERROR("Qz:Cmn:GenParam") << "Could not parse one of the "
                                                   "'config' tags";
                }
            }
            if (configLists.isEmpty()) {
                QZ_ERROR("Qz:Cmn:GenParam") << "Could not find 'config' tags "
                                               "inside 'configs'";
            }
        } else {
            QZ_ERROR("Qz:Cmn:GenParam") << "Could not find root 'configs' tag";
        }
    } else {
        QZ_ERROR("Qz:Cmn:GenParam") << "Failed to read config XML content at "
    ...
    if (configName == "config" && !name.isEmpty() && !id.isEmpty()) {
        ...
        for (auto paramNode = configRoot.firstChildElement("param");
            ...
            auto param = parseParam(
                *config, config.get(), paramNode.toElement());
            if (param != nullptr) {
                config->registerParam(param.get());
                config->addChildParameter(param);
            } else {
                QZ_ERROR("Qz:Cmn:GenParam") << "Failed to parse top level "
        ...
        for (auto groupNode = configRoot.firstChildElement("group");
            ...
            auto group = parseGroup(
                *config, config.get(), groupNode.toElement());
            if (group != nullptr) {
                config->addGroup(group);
            } else {
                QZ_ERROR("Qz:Cmn:GenParam")
                    << "Failed to parse a high level group for config " << name;
            }
        }
    } else {
...
std::shared_ptr<Param> ConfigParser::parseParam(Config& config,
                                                TreeNode* parent,
    ...
    if (type == ParamType::Boolean) {
        ...
        auto bparam = std::make_shared<BooleanParam>(id, name, desc, parent);
        ...
        auto cparam = std::make_shared<ChoiceParam>(id, name, desc, parent);
        ...
        if (min < max && inc != 0) {
            auto rparam = std::make_shared<RangeParam>(id, name, desc, parent);
        ...
        auto tparam = std::make_shared<TextParam>(id, name, desc, parent);
...
std::shared_ptr<Group> ConfigParser::parseGroup(Config& config,
                                                TreeNode* parent,
    ...
    if (!id.isEmpty() && !name.isEmpty()) {
        group = std::make_shared<Group>(id, name, desc, parent);
        for (auto pn = groupNode.firstChildElement("param"); !pn.isNull();
        ...
        for (auto gn = groupNode.firstChildElement("group"); !gn.isNull();
