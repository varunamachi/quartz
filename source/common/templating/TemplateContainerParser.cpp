...
QVector<std::shared_ptr<Template>> TemplateContainerParser::parse(
    ...
    if (content.isNull()) {
        QZ_ERROR("Qz:Cmn:Tmpl") << "Invalid content given to general param "
                                   "parser";
        return tmpls;
    }
    QDomDocument doc;
        ...
        if (root.tagName() == "templates") {
            ...
            for (auto i = 0; i < tmplLists.size(); ++i) {
                ...
                if (tmpl != nullptr) {
                    ...
                    QZ_ERROR("Qz:Cmn:Tmpl") << "Could not parse one of the "
                                               "'template' tags";
                }
            }
            if (tmplLists.isEmpty()) {
                QZ_ERROR("Qz:Cmn:Tmpl") << "Could not find 'template' tags "
                                           "inside 'templates'";
            }
        } else {
            QZ_ERROR("Qz:Cmn:Tmpl") << "Could not find root 'templates' tag";
        }
    } else {
        QZ_ERROR("Qz:Cmn:Tmpl") << "Failed to read config XML content at "
...
std::unique_ptr<Config> parseConfig(const QDomElement& root) {
    ...
    if (configNodeList.size() == 1) {
        ...
        if (config == nullptr) {
            QZ_ERROR("Qz:Cmn:Tmpl") << "Failed to parse template parameters";
        }
    } else {
            ...
            if (child.isCDATASection()) {
                ...
                if (file.open(QFile::ReadOnly)) {
                    ...
                    QZ_ERROR("Qz:Cmn:Tmpl")
                        << "Failed to open template file at '" << path << "'";
                }
            }
        }
        if (content.size() != 0) {
...
std::unique_ptr<Template> TemplateContainerParser::parse(
    ...
    if (!name.isEmpty()) {
        ...
        if (config != nullptr) {
            ...
            if (count == 0) {
                QZ_ERROR("Qz:Cmn:Tmpl") << "Found empty template content";
            }
        } else {
            QZ_ERROR("Qz::Cmn:Tmpl") << "Could not find valid config in "
                                        "tempalte container";
        }
    } else {
        QZ_ERROR("Qz:Cmn:Tmpl") << "Could not read the template name attribute";
    }
    return tmpl;
}
