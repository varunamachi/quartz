...
SelectorManager::SelectorManager(AbstractContainer* container, QWidget* parent)
    : QWidget(parent)
...
void SelectorManager::removeSelector(AbstractSelector* selector) {
    if (selector != nullptr) {
        ...
        QZ_INFO("Qz:SelectorManager") << "succesfully removed selector with id "
                                      << selector->selectorId();
    } else {
        QZ_ERROR("Qz:SelectorManager") << "Could not remove selector with id "
...
void SelectorManager::onSelectorSelected(const QString& selectorId,
                                         QWidget* /*widget*/) {
    auto selector = m_data->m_selectors.value(selectorId, nullptr);
    if (selector != nullptr) {
        selector->selected();
    }
}
