...
struct Node::Data {
    Data(Node* parent, const QString& nodeName, const QString& id, QIcon&& icon)
        : m_parent(parent)
        , m_id(id)
        , m_nodeName(nodeName)
...
Node::Node(Node* parent,
    ...
    : m_data(
        std::make_unique<Data>(parent, nodeName, nodeId, std::move(icon))) {
}
...
//            QIcon icon)
////    : m_data(std::make_unique<Data>(parent, nodeId, std::move(icon))
//    : m_data(std::make_unique<Data>(nullptr, nodeId, nodeName,
//    std::move(icon)))
//{
