...
const QPixmap& icon(NotificationType type) {
    if (type == NotificationType::Info) {
        static auto INFO = getIcon(MatIcon::InfoOutline, Qt::white)
                               .pixmap(ICON_SIZE);
        return INFO;
    } else if (type == NotificationType::Warning) {
        static auto WARN = getIcon(MatIcon::Warning, Qt::white)
                               .pixmap(ICON_SIZE);
        return WARN;
    }
    static auto ERR = getIcon(MatIcon::Error, Qt::white).pixmap(ICON_SIZE);
    return ERR;
}

struct NotificationWidget::Data {
    Data(std::shared_ptr<Msg>&& msg, NotificationWidget* /*widget*/)
        : m_msg(msg) {
    }
...
NotificationWidget::NotificationWidget(std::shared_ptr<Msg> msg,
                                       QWidget* parent)
    : QWidget(parent)
    ...
    connect(
        closeBtn, &QPushButton::released, this, &NotificationWidget::closed);
    this->setLayout(ml);

    auto qss = QString("background-color: %1; color: white;")
                   .arg(m_data->color().name(QColor::HexArgb));
...
void NotificationWidget::paintEvent(QPaintEvent* event) {
    ...
    painter.drawRoundedRect(
        rect.x() + 1, rect.y() + 1, rect.width() - 2, rect.height() - 2, 5, 5);
}
