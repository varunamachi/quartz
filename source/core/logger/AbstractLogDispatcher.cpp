...
public:
    ...
    std::unordered_map<std::string, std::unique_ptr<AbstractLogTarget>>
        m_ownedTargets;

    std::unordered_map<std::string, std::unique_ptr<ILogFilter>> m_ownedFilters;
...
bool AbstractLogDispatcher::Impl::addTarget(AbstractLogTarget* target) {
    ...
    if (target != nullptr && !m_targets.contains(target->uniqueId())) {
        ...
        m_targets.insert(targetId, std::make_shared<TargetInfo>(target, true));
        result = true;
    }
    return result;
}
...
bool AbstractLogDispatcher::Impl::setTargetEnabledState(const QString& trgId,
                                                        bool value) {
    bool result = false;
...
bool AbstractLogDispatcher::Impl::installFilter(ILogFilter* filter,
    ...
    auto getFilterInfo = [&]() -> FilterInfo& {
        if (m_allFilters.contains(filter->filterId())) {
            ...
            m_allFilters.insert(
                filterId, std::make_shared<FilterInfo>(std::move(filter)));
