...
const QString CREATE_QUERY = "CREATE TABLE Config ("
                             "    key            TEXT,"
                             "    value          BLOB,"
                             "    domain         TEXT,"
                             "PRIMARY KEY(key, domain))";

const QString RETRIEVE_QUERY = "SELECT value FROM Config WHERE key = ? AND "
                               "domain = ?";

const QString REMOVE_QUERY = "DELETE FROM Config WHERE key = ? AND domain = ?";

const QString STORE_QUERY = "INSERT OR REPLACE INTO Config(key, value, domain) "
                            "VALUES(?, ?, ?)";
...
DefaultStorageStrategy::DefaultStorageStrategy(const QString& dbPath)
    ...
    if (db.open()) {
        ...
        if (query.exec() && query.next()) {
            ...
            QZ_INFO("Qz:Core:Config") << "Creating database at " << dbPath;
            if (m_data->createSchema()) {
                QZ_INFO("Qz:Core:Config") << "Database creation successfull";
                m_data->m_valid = true;
            } else {
                QZ_ERROR("Qz:Core:Config")
                    << "Database creation failed: " << db.lastError().text();
            }
        }
    } else {
        QZ_ERROR("Qz:Core:Config")
...
bool DefaultStorageStrategy::remove(const QString& domain, const QString& key) {
    QSqlQuery query;
...
QVariantHash DefaultStorageStrategy::allFromDomain(const QString& domain,
                                                   BlobDecoder decoder) const {
    QVariantHash values;
    const auto q = QStringLiteral("SELECT * FROM Config WHERE domain = ?");
    QSqlQuery query;
    query.prepare(q);
    query.addBindValue(domain);
    bool result = query.exec();
