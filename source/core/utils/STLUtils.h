...
namespace Quartz { namespace STLUtils {
...
struct IsStdContainer {
    template <
        typename U,
        typename = typename EvalHelper<typename U::value_type,
                                       typename U::iterator,
...
struct IsAssociativeContainer {
    template <typename U,
              typename = typename EvalHelper<typename U::key_type,
...
void eraseif(
    ...
    auto it = std::remove_if(
        std::begin(container), std::end(container), condition);
...
void eraseif(
    ContainerType& container,
    std::function<bool(const typename ContainerType::mapped_type&)> condition) {
...
void remove(ContainerType& container,
    ...
    auto it = std::remove(std::begin(container), std::end(container), item);
    container.erase(it, std::end(container));
}
