...
struct PluginInfo {
    ...
    PluginInfo(Plugin* plugin, std::shared_ptr<QLibrary> library)
        : m_plugin(plugin)
        , m_library(library) {
    }
...
bool PluginManager::Impl::load(const QDir& dir) {
    ...
    for (auto it = loadedPlugins.begin(); it != loadedPlugins.end(); ++it) {
        ...
        if (!processed.contains(pinfo.m_plugin->pluginId())) {
            result = processPlugins(pinfo, loadedPlugins, processed) && result;
        }
    }
        ...
        if (!m_plugins.contains(pinfo.m_plugin->pluginId())) {
            ...
            if (lib != nullptr) {
                ...
                if (destroyFunc != nullptr) {
                    ...
                    QZ_DEBUG("Qz:Core:Ext") << "Could not find symbol for "
                                               "destroy function in "
                                               " library: "
                                            << lib->fileName();
                }
            ...
            QZ_DEBUG("Qz:Core:Ext")
                ...
                << pinfo.m_plugin->pluginId() << " which failed to initialize";
        } else {
...
std::size_t PluginManager::Impl::loadPlugins(
    ...
    auto fileList = pluginDir.entryInfoList(
        extensions, QDir::Files, QDir::NoSort);
    ...
    if (lib->isLoaded()) {
        ...
        if (func != nullptr) {
            ...
            if (wrapper.enabled && wrapper.plugin != nullptr) {
                ...
                QZ_WARN("Qz:Core:Ext") << "Library at " << pluginFilePath
                                       << " does not "
...
bool PluginManager::Impl::processPlugins(
    ...
    for (auto i = 0; i < hardDeps.size(); ++i) {
        ...
        if (plgInfo.isValid()) {
            result = processPlugins(plgInfo, loadedPlugins, processedPlugins);
        } else {
            result = false;
        }
        if (!result && plgEntry.second == DependencyType::Optional) {
            ...
            QZ_ERROR("Qz:Core:Ext") << "Could not initialize plugin with id "
                                    << pluginInfo.m_plugin->pluginId()
                                    << "; could not find required dependncy "
                                       "with id "
                                    << plgEntry.first;
            break;
        }
    ...
    if (result) {
        ...
        QZ_DEBUG("Qz:Core:Ext") << "Successfuly processed plugin with id "
                                << pluginInfo.m_plugin->pluginId();
    }
    ...
    foreach (auto& plugin, m_plugins) {
        ...
        if (lib != nullptr) {
            ...
            if (destroyFunc != nullptr) {
                ...
                QZ_DEBUG("Qz:Core:Ext") << "Could not find symbol for destroy "
                                           "function in "
                                           " library: "
                                        << lib->fileName();
            }
...
void PluginManager::registerPluginAdapter(IExtensionAdapter* adapter) {
    std::shared_ptr<IExtensionAdapter> adptr{adapter, [](IExtensionAdapter*) {
                                             }};
...
QVector<const Plugin*> PluginManager::plugins() const {
    ...
    for (auto it = m_impl->m_plugins.begin(); it != m_impl->m_plugins.end();
         ++it) {
...
const QLibrary* PluginManager::libraryForPlugin(const QString& pluginId) const {
